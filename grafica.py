...
def iniciar_bot():
    ...
    def iniciar_script():
        ...
        while True:
            agora = datetime.now().strftime('%H:%M')
            if agora > HORA_FIM:
                print(f'Fim do horário de {HORA_FIM} agora são {agora}')
                salvar_print_csv(f'Fim do horário de {HORA_FIM} agora são {agora}')
                print('')
                break
    ...
    button = Button(root, text="Iniciar Script", command=iniciar)
    button.pack(pady=10)
